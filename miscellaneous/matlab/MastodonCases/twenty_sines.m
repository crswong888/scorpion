...
dt = 1e-03 ; % A.K. Chopra rule floored to nearest one thousandth
T = 20 / frequency ; % 20 cycles
t = 0:dt:T ;
NT = length(t) ;
...
accel_fit_order = 10 ; % approximation order = N - 1 (SHOULD BE <= 10)
vel_fit_order = 10 ;
disp_fit_order = 8 ;
...
d2u = zeros(1,NT) ;
for i = 1:NT
    d2u(i) = -amplitude * (2 * pi * frequency) * (2 * pi * frequency) * sin(2 * pi * frequency * t(i)) ;
end
