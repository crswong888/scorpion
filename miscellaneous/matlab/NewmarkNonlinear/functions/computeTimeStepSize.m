...
function dt = computeTimeStepSize(t, omega_n, p, varargin)
    ...
    %// compute time step size using a coefficient of '1 / 10' for linear and '1 / 20' for nonlinear
    dt = (1 / 10 - params.Results.Nonlinear / 20) * 2 * pi / omega;
    
    %/ round 'dt' down to nearest half order of magnitude to ensure that it's a clean number
...
function omegaBar = maxForcingFrequency(t, p)
    ...
    %// now perform a FFT between time indices 'i' and 'j' along angular frequecy domain
    [omega_p, FFT] = discreteFourierTransform(t(i:j), p(i:j), 'FrequencyType', 'angular');
    
    %/ get local maxima frequencies - ignore small amps likely to be mere artifacts of FFT algorithm
    [po, o] = findpeaks([0, FFT, 0]); % pad zeros to help 'findpeaks()' algorithm
    
    %// now find maximum freequency of significant amplitudes and return
